...
from .schemas import TodoUpdate, UserCreate, TodoCreate
...
def create_user(session: Session, user: UserCreate) -> models.User:
    try:
...
def create_user_todo(session: Session, todo: TodoCreate, user_id: int) -> models.Todo:
    try:
...
def update_todo(session: Session, db_todo: models.Todo, todo_in: TodoUpdate) -> models.Todo:
    try:
        # Use model_dump() with Pydantic v2+
